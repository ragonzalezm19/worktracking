...
<!-- start: SCRIPT -->
	...
	<script src="<?php echo $Path; ?>assets/plugins/bootstrap-datepicker/js/bootstrap-datepicker.js"></script>
	...
	<script>
			...
			if (Validacion()){
				parametros:{
					parametros = {
						...
						"fecha"      : $('#fecha').val(),
						"rh_id"      : $('#personal-asignado').val()
					}
					// alert(JSON.stringify(parametros));
					jQuery.ajax({
						data: parametros,
						url: '<?php echo $Path ?>assets/ajax/Registro/Proyectos/add.php',
						type: 'post',
						dataType: 'json',
						success: function(respuesta){ 
		...
		$('a[href="#editar"]').click(function () 
		{
			parametros:{
				...
				jQuery.ajax({
					data: parametros,
					url: '<?php echo $Path ?>assets/ajax/Registro/Proyectos/edit.php',
					type: 'post',
					dataType: 'json',
					success: function(respuesta){
						$('#modal-title').empty().append('<i class="clip-screen teal"></i> Proyecto No. ' + respuesta['id']);
						...
						$('#personal-asignado_e').select2("val",respuesta['rh_id']);
						$('#etapa-del-proyecto_e').select2("val",respuesta['proyecto_estado_id']);
			...
			if (ValidacionEditar()){
				parametros:{
					parametros = {
						...
						"costo"             : $('#costo_e').val(),
						"cliente"           : $('#cliente_e').val(),
						"rh_id"             : $('#personal-asignado_e').val(),
						"etapa-del-proyecto": $('#etapa-del-proyecto_e').val()
					}
					// alert(JSON.stringify(parametros));
					jQuery.ajax({
						data: parametros,
						url: '<?php echo $Path ?>assets/ajax/Registro/Proyectos/update.php',
						type: 'post',
						dataType: 'json',
						success: function(respuesta){
							location.reload();
		...
		$('a[href="#estado"]').click(function () 
		{
			parametros:{
				...
				jQuery.ajax({
					data: parametros,
					url: '<?php echo $Path ?>assets/ajax/Registro/Proyectos/status.php',
					type: 'post',
					dataType: 'json',
					success: function(respuesta){
		...
		$('button#estado-accion').click(function () 
		{
			parametros:{
				...
				jQuery.ajax({
					data: parametros,
					url: '<?php echo $Path ?>assets/ajax/Registro/Proyectos/update-status.php',
					type: 'post',
					dataType: 'json',
					success: function(respuesta){
						location.reload();
	...
	<script>
		...
		$("#cliente, #cliente_e").select2({
			placeholder: "Seleccione un cliente",
			allowClear: true
		});
		$("#personal-asignado, #personal-asignado_e").select2({
			placeholder: "Seleccione una persona",
			allowClear: true
		});
		$("#etapa-del-proyecto_e").select2({
