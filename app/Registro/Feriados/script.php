	...
	<script>
		...
		$('button#agregar').click(function () 
		{
			if (Validacion()){
				parametros:{
					...
					jQuery.ajax({
						data: parametros,
						url: '<?php echo $Path ?>assets/ajax/Registro/Feriados/add.php',
						type: 'post',
						dataType: 'json',
						success: function(respuesta){ 
							location.reload();
		...
		$('a[href="#editar"]').click(function () 
		{
			parametros:{
				...
				jQuery.ajax({
					data: parametros,
					url: '<?php echo $Path ?>assets/ajax/Registro/Feriados/edit.php',
					type: 'post',
					dataType: 'json',
					success: function(respuesta){
						$('#modal-title').empty().append('<i class="flaticon-business61 teal"></i> Feriado No. ' + respuesta['id']);
						$('#nombre_e').val('').val(respuesta['nombre']);
						var f = respuesta['fecha'].split('-');
						$('#fecha_e').val('').val(f[2]+'-'+f[1]+'-'+f[0]);
		...
		$('#actualizar').click(function () 
		{
			if (ValidacionEditar()){
				parametros:{
					...
					jQuery.ajax({
						data: parametros,
						url: '<?php echo $Path ?>assets/ajax/Registro/Feriados/update.php',
						type: 'post',
						dataType: 'json',
						success: function(respuesta){
							location.reload();
		...
		$('a[href="#estado"]').click(function () 
		{
			parametros:{
				...
				jQuery.ajax({
					data: parametros,
					url: '<?php echo $Path ?>assets/ajax/Registro/Feriados/status.php',
					type: 'post',
					dataType: 'json',
					success: function(respuesta){
		...
		$('button#estado-accion').click(function () 
		{
			parametros:{
				...
				jQuery.ajax({
					data: parametros,
					url: '<?php echo $Path ?>assets/ajax/Registro/Feriados/update-status.php',
					type: 'post',
					dataType: 'json',
					success: function(respuesta){
						location.reload();
