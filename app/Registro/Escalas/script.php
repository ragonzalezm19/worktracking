...
<!-- start: SCRIPT -->
	...
	<script src="<?php echo $Path; ?>assets/plugins/bootstrap-daterangepicker/moment.min.js"></script>
	<script src="<?php echo $Path; ?>assets/plugins/bootstrap-daterangepicker/daterangepicker.js"></script>
	<script>
	...
	<script>
		...
		$('button#agregar').click(function () 
		{
			parametros:{
				parametros = {
					...
					"desde"      : $('#rangotiempo').val().split(" - ")[0],
					"hasta"      : $('#rangotiempo').val().split(" - ")[1],
					"abreviatura": $('#abreviatura').val()
				}
				// alert(JSON.stringify(parametros));
				jQuery.ajax({
					data: parametros,
					url: '<?php echo $Path ?>assets/ajax/Parametros/Escalas/add.php',
					type: 'post',
					dataType: 'json',
					success: function(respuesta){ 
						location.reload();
		...
		$('a[href="#editar"]').click(function () 
		{
			parametros:{
				...
				jQuery.ajax({
					data: parametros,
					url: '<?php echo $Path ?>assets/ajax/Parametros/Escalas/edit.php',
					type: 'post',
					dataType: 'json',
					success: function(respuesta){
						$('#modal-title').empty().append('<i class="flaticon-market teal"></i> Escala No. ' + respuesta['id']);
						$('#nombre_e').val('').val(respuesta['nombre']);
						$('#descripcion_e').val('').val(respuesta['descripcion']);
						$('#rangotiempo_e').val('').val(respuesta['desde'] + ' - ' + respuesta['hasta']);
		...
		$('#actualizar').click(function () 
		{
			parametros:{
				parametros = {
					...
					"desde"      : $('#rangotiempo_e').val().split(" - ")[0],
					"hasta"      : $('#rangotiempo_e').val().split(" - ")[1],
					"abreviatura": $('#abreviatura_e').val()
				}
				// alert(JSON.stringify(parametros));
				jQuery.ajax({
					data: parametros,
					url: '<?php echo $Path ?>assets/ajax/Parametros/Escalas/update.php',
					type: 'post',
					dataType: 'json',
					success: function(respuesta){
						location.reload();
		...
		$('a[href="#estado"]').click(function () 
		{
			parametros:{
				...
				jQuery.ajax({
					data: parametros,
					url: '<?php echo $Path ?>assets/ajax/Parametros/Escalas/status.php',
					type: 'post',
					dataType: 'json',
					success: function(respuesta){
		...
		$('button#estado-accion').click(function () 
		{
			parametros:{
				...
				jQuery.ajax({
					data: parametros,
					url: '<?php echo $Path ?>assets/ajax/Parametros/Escalas/update-status.php',
					type: 'post',
					dataType: 'json',
					success: function(respuesta){
						location.reload();
	...
	<script>
		...
		//$('#rangotiempo').daterangepicker();
		$('#rangotiempo').daterangepicker({
			timePicker: true,
			timePickerIncrement: 15,
			format: 'DD/MM/YYYY h:mm A'
		 });
		$('#rangotiempo_e').daterangepicker({
			timePicker: true,
			timePickerIncrement: 15,
			format: 'DD/MM/YYYY h:mm A'
		 });
